...
function ListEquipes() {
  ...
  {equipes.map((equipe) => (
    ...
    <tr>
      ...
      <Button variant="info" size="sm" onClick={()=> {handleShowModifier(equipe.id)}}><FiIcons.FiEdit style={{fontSize:'20px'}}/></Button>{''}<Button variant="danger" size="sm" onClick={()=> {handleShowDelete(equipe.id);}} ><MdIcons.MdDeleteForever style={{fontSize:'20px'}} /></Button>
      </td>
    </tr>
  ))}
