...
<tr>
  ...
  <td>{compte.equipe.nom}</td>
  


  <td style={{width:'275px'}}>
  <Button variant="dark" size="sm"  onClick={() => handleShowAfficher(compte.id)}><BiIcons.BiShowAlt style={{fontSize:'20px'}} /></Button>{''}<Button variant="info" size="sm" onClick={()=> {handleShowModifier(compte.id)}}><FiIcons.FiEdit style={{fontSize:'20px'}}/> </Button>{''}<Button variant="danger" size="sm" onClick={()=> {handleShowDelete(compte.id);}} ><MdIcons.MdDeleteForever style={{fontSize:'20px'}} /></Button>
  </td>
</tr>
))}
</tbody>
    ...
    <Modal show={showAfficher} size="lg" onHide={handleCloseAfficher}>
        ...
        <Modal.Body>
          ...
          <div className="form-group row">
           <label className="col-form-label col-lg-4">Equipe :</label>
            <div className="col-lg-4">
            <input className="form-control" type="text" defaultValue={compte.equipe.nom} readOnly="readonly" /></div>
          </div>
      ...
      <form onSubmit={handleSubmit}>
        <Modal.Header closeButton style={{backgroundColor:'#33b5e5'}}>
          <Modal.Title style={{color:'white'}}>Modifier : {compte.id}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
