...
function ListProjets() {
  ...
  {projets.map((unprojet) => (
    ...
    <tr>
      ...
      <Button variant="dark" size="sm"  onClick={() => handleShowAfficher(unprojet.id)}><BiIcons.BiShowAlt style={{fontSize:'20px'}} /></Button>{''}<Button variant="info" size="sm" onClick={()=> {handleShowModifier(unprojet.id)}}><FiIcons.FiEdit style={{fontSize:'20px'}}/></Button>{''}<Button variant="danger" size="sm" onClick={()=> {handleShowDelete(unprojet.id);}}><MdIcons.MdDeleteForever style={{fontSize:'20px'}} /></Button>
      </td>
    </tr>
  ))}
