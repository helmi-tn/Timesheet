...
function AddEquipe() {
  ...
  const handleChangeSelect = ((e) => {
    let value = Array.from(e.target.selectedOptions,(option) => JSON.parse(option.value))
    setPostDataEquipe({...postDataEquipe, collaborateurs: value})
  ...
  const [postDataEquipe,setPostDataEquipe] = useState({
    id:'',nom:'',responsable:'', collaborateurs:[]
})
    ...
    <div style={{fontSize:'20px',textAlign:'center'}}><b>Ajouter une nouvelle équipe</b></div>
      <hr/>
        <Form onSubmit={handleSubmit}>
          ...
          <Form.Select onClick={(e) =>setPostDataEquipe({...postDataEquipe, responsable: e.target.value})} aria-label="Default select example">
          ...
          <Form.Select multiple="multiple" onChange={(e)=> handleChangeSelect(e)} aria-label="Default select example">
          {collaborateurs.map((collab) => (
              <option value={JSON.stringify(collab)} >{collab.nom} {collab.prenom}</option> 
            ))}
          </Form.Select>
          <br/><br/><br/>
          <center>
